...
func NewVideoMergeHandler(db *gorm.DB, transferService *services2.ResourceTransferService, storagePath, baseURL string, log *logger.Logger, cfg *config.Config) *VideoMergeHandler {
	return &VideoMergeHandler{
		mergeService: services2.NewVideoMergeService(db, transferService, storagePath, baseURL, log, cfg),
		log:          log,
	}
}
