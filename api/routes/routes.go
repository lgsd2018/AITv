...
func SetupRouter(cfg *config.Config, db *gorm.DB, log *logger.Logger, localStorage interface{}) *gin.Engine {
	...
	aiService := services2.NewAIService(db, log, cfg)
	aiHandler := handlers2.NewAIHandler(aiService, log)
	...
	videoMergeHandler := handlers2.NewVideoMergeHandler(db, nil, cfg.Storage.LocalPath, cfg.Storage.BaseURL, log, cfg)
