...
func (s *StoryboardService) GenerateStoryboard(episodeID string, model string) (string, error) {
	...
	err := s.db.Table("episodes").
		Select("episodes.id, episodes.script_content, episodes.description, episodes.drama_id, dramas.style_prompt").
	...
	// 构建风格提示词部分
	stylePromptPart := ""
	if episode.StylePrompt != nil && *episode.StylePrompt != "" {
		stylePromptPart = fmt.Sprintf("\n【项目整体风格要求】\n%s\n请确保分镜描述（尤其是环境氛围、画面结果）符合上述风格要求。\n", *episode.StylePrompt)
	}
...
- 描述光线、色彩、质感、动态
- 为视频生成AI提供足够的画面构建信息
- 避免抽象词汇，使用具象的视觉化描述`, systemPrompt, scriptLabel, scriptContent, taskLabel, taskInstruction, charListLabel, characterList, charConstraint, sceneListLabel, sceneList, sceneConstraint, stylePromptPart, scriptContent)
...
func (s *StoryboardService) generateImagePrompt(sb Storyboard, stylePrompt *string) string {
	var parts []string
	...
	if stylePrompt != nil && *stylePrompt != "" {
...
func (s *StoryboardService) generateVideoPrompt(sb Storyboard, stylePrompt *string) string {
	var parts []string
	...
	if stylePrompt != nil && *stylePrompt != "" {
		parts = append(parts, fmt.Sprintf("Project style: %s", *stylePrompt))
	}
	// 10. 视频比例
...
func (s *StoryboardService) saveStoryboards(episodeID string, storyboards []Storyboard) error {
	...
	return s.db.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Select("style_prompt").Where("id = ?", episode.DramaID).First(&drama).Error; err == nil {
			stylePrompt = drama.StylePrompt
		}
		...
		for _, sb := range storyboards {
			...
			// 生成两种专用提示词
			imagePrompt := s.generateImagePrompt(sb, stylePrompt)
			videoPrompt := s.generateVideoPrompt(sb, stylePrompt)
...
func (s *StoryboardService) CreateStoryboard(req *CreateStoryboardRequest) (*models.Storyboard, error) {
	...
	var stylePrompt *string
	var episode models.Episode
	if err := s.db.Select("drama_id").Where("id = ?", req.EpisodeID).First(&episode).Error; err == nil {
		var drama models.Drama
		if err := s.db.Select("style_prompt").Where("id = ?", episode.DramaID).First(&drama).Error; err == nil {
			stylePrompt = drama.StylePrompt
		}
	}
