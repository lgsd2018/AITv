...
func (s *ImageGenerationService) GenerateImage(request *GenerateImageRequest) (*models.ImageGeneration, error) {
	...
	// Apply StylePrompt globally (if not already present)
	if drama.StylePrompt != nil && *drama.StylePrompt != "" {
		if !strings.Contains(request.Prompt, *drama.StylePrompt) {
			request.Prompt += ", " + *drama.StylePrompt
		}
	}

	// 强制应用角色生成规范（三视图、白底）
	if request.ImageType == string(models.ImageTypeCharacter) {
		enforcement := ", character sheet, three views (front view, side view, back view), white background, full body shot, high quality, 8k resolution"
		if !strings.Contains(request.Prompt, "three views") {
			request.Prompt += enforcement
		}

		// 应用项目风格
		if drama.Style != "" && !strings.Contains(request.Prompt, drama.Style) {
			request.Prompt += ", " + drama.Style + " style"
		}

		// 应用项目比例 (优先使用Drama.AspectRatio)
